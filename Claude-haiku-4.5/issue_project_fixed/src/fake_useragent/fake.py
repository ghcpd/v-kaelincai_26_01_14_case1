...
class FakeUserAgent:
    ...
    def getBrowser(self, browsers: Union[str, list[str]]) -> BrowserUserAgentData:
        ...
        try:
            ...
            # FIX: Use weighted random selection based on percent field
    ...
    def _weighted_choice(
        ...
        """Select a random item from a list using weighted probability based on percent field.

        This fixes Issue #446 by ensuring that user agents are selected based on their
        usage percentage, providing better randomization and reducing duplicates.

        Args:
            choices (list[BrowserUserAgentData]): List of user agent data to choose from.

        Returns:
            BrowserUserAgentData: A randomly selected user agent, weighted by percent.

...
def load() -> list[BrowserUserAgentData]:
    ...
    This function also normalizes the loaded data by removing duplicate entries that
    share the exact same `useragent` string. Duplicate lines are common in the
    bundled dataset and were the root cause of Issue #446: selecting a random
    entry frequently produced identical user-agent strings because multiple
    data rows contained the same `useragent`.
    ...
    # Deduplicate entries by the `useragent` field while preserving file order.
    seen: set[str] = set()
    unique_data: list[BrowserUserAgentData] = []
    duplicates = 0
    for entry in data:
    ...
    if duplicates:
        logger.debug("Removed %d duplicate useragent entries from bundled data", duplicates)
